...
(function ($){
    var FreePageBuilderLimit = 99999;
    var sourceObject;
    ...
    $.JSNPageBuilder	= function(source_id, params){
        ...
        JSNPageBuilder.init	= function (){
            ...
            if(window.location.search.indexOf('option=com_flexicontent') == 1)
            {
                thisContentId = this.container.find("input[name^='jform[id]']").val()
            }
            if(thisContentId != ''){
                content = 'isEdit';
            }else{
        ...
        JSNPageBuilder.transformSourceValueToHTML	= function (){
            ...
            var isComModules = window.location.search.indexOf('option=com_modules');
            var formId = $('#jform_id').val();
            if(window.location.search.indexOf('option=com_flexicontent') == 1)
            {
                formId = this.container.find("input[name^='jform[id]']").val()
            }
            $.post(JSNPbParams.rootUrl + 'administrator/index.php?option=com_pagebuilder&task=builder.html&'+token+'=1',
                {form_data:  this.source.val(), article_id: formId, is_com_modules: isComModules},
                function (data) {
