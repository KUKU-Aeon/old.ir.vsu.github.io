 ...
 * Класс вида
 */
class PriceleafsViewZakaz extends JViewLegacy
{
	protected $state;
	...
	{
		JFactory::getApplication()->input->set('hidemainzakaz', true);
		...
		JToolbarHelper::title($isNew ? JText::_('COM_PRICELEAF_ZAKAZOPISANIE') : JText::_('COM_PRICELEAF_ZAKAZOPISANIE'), 'priceleafs.png');

		// If not checked out, can save the item.
		if (!$checkedOut && ($canDo->get('core.edit')||(count($user->getAuthorisedCategories('com_priceleafs', 'core.create')))))
		{
			JToolbarHelper::apply('zakaz.apply');
			JToolbarHelper::save('zakaz.save');
		}

		if (empty($this->item->id)) {
			JToolbarHelper::cancel('zakaz.cancel');
		}
		else {
			JToolbarHelper::cancel('zakaz.cancel', 'JTOOLBAR_CLOSE');
		}
