...
class DJLicense{
	...
	public static function getSubscription($name){
		...
		//$app->enqueueMessage('package: '.print_r($result, true));
							...
							if(confirm('".addslashes(JText::sprintf('DJUPDATER_CONFIRM_UPDATE_MESSAGE', 'DJ-'.$name))."')) {
								...
								jQuery.ajax({
									...
									data: {
										...
										install_url: 'http://dj-extensions.com/index.php?option=com_djsubscriptions&view=getUpdate&license=$license&ext=$ext&v=".$version."'
									}
								}).done(function(data) {
	...
	public static function setUpdateServer($name) {
		...
		$config = JFactory::getConfig();
		$secret_file = JFile::makeSafe('license_'.$config->get('secret').'.txt');
		$license_file = JPath::clean(JPATH_BASE.'/components/com_dj'.strtolower($name).'/'.$secret_file);
