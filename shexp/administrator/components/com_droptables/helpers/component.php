...
class droptablesComponentHelper extends JComponentHelper {
    /**
    ...
    public function setParams($datas){
            $component = JComponentHelper::getComponent('com_droptables');
            ...
            unset(self::$components['com_droptables']);
            return true;
    }
    ...
    static public function getManifest(){
        $component = self::getComponent('com_droptables');
        $table	= JTable::getInstance('extension');
        // Load the previous Data
        if (!$table->load($component->id,false)) {
                return false;
