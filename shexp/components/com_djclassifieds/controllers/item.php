...
class DJClassifiedsControllerItem extends JControllerLegacy {
	...
	/*	$menus	= JSite::getMenu();	
		...
		*/		
		...
		$db		= JFactory::getDBO();
			$query = "SELECT date_exp, user_id, published, blocked FROM #__djcf_items WHERE id=".$id;
			$db->setQuery($query);
		$item = $db->loadObject();
		
		$advert_available = true;
		
		if($item->published==0 || $item->blocked==1 || ($item->published==2 && $par->get('show_archived','1')==0)){			
