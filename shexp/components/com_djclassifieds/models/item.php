...
class DjclassifiedsModelItem extends JModelLegacy{	
	...
	function getItem(){
		...
		$db= JFactory::getDBO();
		$query = "SELECT i.*,c.name as c_name,c.alias as c_alias, c.access_view as c_access_view, c.access_item_view as c_access_item_view,  c.rev_group_id, ".$u_name.", u.email as u_email, iu.name as unit_name ".$fav_s.$type_s." FROM #__djcf_categories c, #__djcf_items i "
				.$fav_lj.$type_lj
	...
	function getUserItemsCount($uid){		
		...
		$query = "SELECT COUNT(i.id) FROM #__djcf_items i WHERE i.published=1 AND i.date_exp > '".$date_now."' AND i.user_id  = ".$uid." ";
		$db->setQuery($query);
		$item=$db->loadResult();
		return $item;
	}
