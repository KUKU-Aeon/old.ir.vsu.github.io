...
class DjclassifiedsModelItems extends JModelLegacy{	
	...
	function getItems($catlist=''){
		...
		$cid		= JRequest::getInt('cid',0);
					...
					if(JRequest::getVar('search',JText::_('COM_DJCLASSIFIEDS_SEARCH'),'','string')!=JText::_('COM_DJCLASSIFIEDS_SEARCH') && JRequest::getVar('search','','','string')){
						...
						if($par->get('search_type_phrase',0)==1){
							...
							foreach($se_words as $se_w){
								if(strlen($se_w)>2){
									$search .= " AND (CONCAT_WS(i.name,i.intro_desc,i.description) LIKE ".$db->Quote('%'.$se_w.'%')." OR c.name LIKE ".$db->Quote('%'.$se_w.'%')." OR r.name LIKE ".$db->Quote('%'.$se_w.'%')." OR i.id=".$db->Quote($se_w)." OR ".$u_name_search." LIKE ".$db->Quote($se_w)." ) ";
								}
							}	
						}else{
							...
							$search = " AND (CONCAT_WS(i.name,i.intro_desc,i.description) LIKE ".$search_word." OR c.name LIKE ".$search_word." OR r.name LIKE ".$search_word." OR i.id=".$search_word2." OR ".$u_name_search."=".$search_word2." ) ";
						}
					}
			...
			$query = "SELECT i.*, c.name AS c_name,c.alias AS c_alias, c.id as c_id, c.rev_group_id, r.name as r_name, r.id as r_id ".$search_img_count_v.$fav_s.$search_radius_v.$distance_v.$rev_s." FROM ".$search_fields." #__djcf_items i "
					...
					.$rev_lj
					.$mcat_lj
					."WHERE i.date_exp > '".$date_now."' AND i.published=1 AND i.blocked=0 AND c.published=1 "
					.$where;
