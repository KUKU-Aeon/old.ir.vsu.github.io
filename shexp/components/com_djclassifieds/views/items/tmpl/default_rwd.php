		...
		foreach($this->items as $i){
			...
			if($par->get('column_image','1')){		
				echo '<div class="item_col icon first"  style="width:'.$icon_col_w.'px"  >';						
					echo '<a href="'.DJClassifiedsSEO::getItemRoute($i->id.':'.$i->alias,$i->cat_id.':'.$i->c_alias,$this->main_cat,$i->extra_cats).$anch.'">';
						if(count($i->images)){									
			...
			echo '<div class="item_col name'.$title_column_class.'">';					
				if((int)$par->get('tooltip_title','1')){
					echo '<h3><a class="title Tips1" href="'.DJClassifiedsSEO::getItemRoute($i->id.':'.$i->alias,$i->cat_id.':'.$i->c_alias,$this->main_cat,$i->extra_cats).$anch.'" title="'.$tip_title.'" rel="'.$tip_cont.'" >'.$i->name.'</a></h3>';
				}else{
					echo '<h3><a class="title" href="'.DJClassifiedsSEO::getItemRoute($i->id.':'.$i->alias,$i->cat_id.':'.$i->c_alias,$this->main_cat,$i->extra_cats).$anch.'" >'.$i->name.'</a></h3>';
				}
			...
			if($par->get('column_desc','1')){
				...
				echo '<div class="item_col description'.$class.'"><a class="title" href="'.DJClassifiedsSEO::getItemRoute($i->id.':'.$i->alias,$i->cat_id.':'.$i->c_alias,$this->main_cat,$i->extra_cats).$anch.'">';
			...
			if($par->get('column_loc','1') && $par->get('show_regions','1')){
				...
				echo '<div class="item_col region'.$class.'"><a href="'.$this->se_results_link.$i->region_id.'">'.$i->r_name.'</a></div>';
			}
