...
class DJclassifiedsViewItem extends JViewLegacy{
	...
	function display($tpl = null){		
		...
		if($item->user_id!=0){
			...
			$correct_link = $base_link.JRoute::_(DJClassifiedsSEO::getItemRoute($item->id.':'.$item->alias,$item->cat_id.':'.$item->c_alias)).$anch;
			...
			if($item_images){
				$document->addCustomTag('<meta property="og:image" content="'.$base_link.$item_images[0]->thumb_b.'" />');
