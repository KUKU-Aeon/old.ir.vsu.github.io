				...
				if($item->user_id==0){
					...
					<div class="profile_item_box">
						...
						<div class="profile_name_data">
							<?php echo '<a title="'.JText::_('COM_DJCLASSIFIEDS_SEE_ALL_USER_ADVERTS').'" class="profile_name" href="index.php?option=com_djclassifieds&view=profile&uid='.$uid_slug.DJClassifiedsSEO::getUserProfileItemid().'">'.$item->username.' <span>('.$this->user_items_c.')</span></a>'; ?>
							<?php if($this->profile['data']){ ?>
								...
								<?php foreach($this->profile['data'] as $f){
									...
									<div class="profile_row row_<?php echo $f->name;?>">
										...
										<span class="row_value<?php if($f->hide_on_start){echo ' djsvoc" title="'.htmlentities($f->value); }?>" >
											<?php 
											if($f->type=='textarea'){							
												if($f->value==''){echo '---'; }
												else{echo $f->value;}								
