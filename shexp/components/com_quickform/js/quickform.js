...
function qfclonep(x,m){
	...
	showLabel(d);
	var z=d.getParent('form');
	qfsumBox(z,'htrut',z.qfcod.value);
...
function qfRestorForm(f) {
	...
	var captdiv = f.getElement('.qf_recaptcha');
	
	if(!captchaflagstart && captdiv){
		var e = document.createElement("script");
		e.src = 'https://www.google.com/recaptcha/api.js?onload=qfdocaptcha&render=explicit';
		e.type="text/javascript";
		document.getElementsByTagName("body")[0].appendChild(e); 
		captchaflagstart ++;
	}
	else if(!captchaflag[f] && captdiv) qfdocaptcha();
	else if(captchaflag[f] && !captdiv) captchaflag[f] = 0;
...
function qfdocaptcha() {
	var ff=$$('.qfblock form');
	ff.each(function(f){
		if(!captchaflag[f] && captchaflagstart<100){
			if(typeof(grecaptcha)!='object'){
				setTimeout(qfdocaptcha, 300);
				captchaflagstart ++;
			}
			else{
				var captdiv = f.getElements('.qf_recaptcha')[0];
				captchaflag[f] = 1;
				grecaptcha.render(captdiv, {
						'sitekey' : captdiv.getAttribute("data-sitekey"),
						'theme' : captdiv.getAttribute("data-theme")
				});
			}
		}
	});
