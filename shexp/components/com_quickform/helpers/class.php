...
//				if($svz){echo $svz;die;}
	...
	function getFilds($settlement,$c, $reqid, $tmpl='default') 
	{
		...
		foreach($arr as $ar) {
			...
			elseif(strpos($ar,'>captcha</td>')!== false) $this->getCaptcha($ar,$this->post,$c);
		}
	...
	function getCaptcha($pat,$post,$c) 
	{
		...
		if($user->get('guest')){
			...
			if (empty($privatekey) || empty($remoteip) || $spam)
			{
				$app->redirect($_SERVER['HTTP_REFERER'], JText::_('RECAPTCHA_ERROR'), 'error');
			}
			require_once JPATH_BASE . '/plugins/captcha/recaptcha/recaptchalib.php';
			...
			if ( !isset($response->success) || !$response->success)
			{
				...
				{
					$app->redirect($_SERVER['HTTP_REFERER'], $error, 'error');
				}
			}
		}
	}
