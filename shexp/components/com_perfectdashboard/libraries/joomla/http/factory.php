...
if (!class_exists('JHttpFactory'))
{
	...
	JLoader::register('JHttpFactory', JPATH_LIBRARIES_DASHBOARD.'/joomla/http/factory.php', true);
...
class JHttpFactory
{
	...
	public static function getAvailableDriver(JRegistry $options, $default = null)
	{
		...
		foreach ($availableAdapters as $adapter)
		{
			...
			try
			{
				//PWeb only construct object and do not call method isSupported() which may not exist
	...
	{
		$names = array();
		$iterator = new DirectoryIterator(JPATH_LIBRARIES . '/joomla/http/transport'); //PWeb changed location
