...
class modDjClassifiedsItems{
	public static function getItems($params){
		...
		$query = "SELECT i.*,c.id as c_id, c.name as c_name,c.alias as c_alias,c.icon_url as c_icon_url, r.id as r_id, r.name as r_name "
				...
				."WHERE i.date_exp > '".$date_now."' AND i.published = 1 AND i.blocked=0 AND c.published = 1 AND i.cat_id=c.id "
				.$promoted.$item_ids.$users_ids.$fallow_cat.$cat_list.$fallow_region.$region_list.$follow_user.$types_ids.$only_img.$source.$access_view.$current_ad.$adult_restriction
	...
	static function getFields($params,$items){
		...
		$query ="SELECT f.*, v.value,v.value_date,v.item_id FROM #__djcf_fields f "
